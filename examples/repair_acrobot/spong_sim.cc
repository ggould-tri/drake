...
struct Scenario {
  ...
  Eigen::Vector4d controller_params = Eigen::Vector4d::Constant(NAN);
  Eigen::Vector4d initial_state = Eigen::Vector4d::Constant(NAN);
  double t_final = NAN;
...
std::string Simulate(const YAML::Node& scenario_node) {
  ...
  auto& controller_context = diagram->GetMutableSubsystemContext(
      *controller, &context);
  plant_context.SetContinuousState(scenario.initial_state);
...
int main(int argc, char* argv[]) {
  ...
  return drake::examples::repair_acrobot::Main();
}
